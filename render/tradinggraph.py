...
CLOSE_UP_TEXT_COLOR = '#FFFF00'
CLOSE_DOWN_TEXT_COLOR = '#FFFF00'
...
class TradingGraph:
    ...
    def __init__(self, df, initial_net_worth, title=None):
        self.df = df
        self.net_worths = np.zeros(len(df['Date']))
        self.net_worths[0] = initial_net_worth
        self.balances = np.zeros(len(df['Date']))

        # Create a figure on screen and set the title
        self.fig = plt.figure()
        self.title = title
        self.fig.suptitle(f'{self.title} | Balance: 0.00 € | Open P/L: 0.00 € | Profit: 0.00%')
    ...
    def _render_net_worth(self, current_step, net_worth, step_range, dates):
        ...
        self.net_worth_ax.plot_date(
            dates, self.balances[step_range], '-', label='Balance', color='green')

        # Plot net worths
        self.net_worth_ax.plot_date(
            dates, self.net_worths[step_range], '-', label='Net Worth', color='red')
        ...
        self.net_worth_ax.set_ylim(
            min(self.net_worths[np.nonzero(self.net_worths)]) / 1.025, max(self.net_worths) * 1.025)
    ...
    def _render_price(self, current_step, net_worth, dates, step_range):
        ...
        candlestick(self.price_ax, candlesticks, width=0.005,
        ...
        self.price_ax.annotate('{0:.5f}'.format(last_close), (last_date, last_close),
    ...
    def _render_volume(self, current_step, net_worth, dates, step_range):
        ...
        self.volume_ax.bar(dates[pos], volume[pos], color=UP_COLOR,
                           alpha=0.4, width=0.005, align='center')
    ...
    def _render_trades(self, current_step, trades, step_range):
        for trade in trades:
            if trade['step'] in step_range:
                ...
                open = self.df['Open'].values[trade['step']]
                ...
                elif trade['type'] == 'close_buy':
                    high_low = open
                    color = CLOSE_UP_TEXT_COLOR
                else:
                    high_low = open
                    color = CLOSE_DOWN_TEXT_COLOR
    ...
    def render(self, current_step, balance, net_worth, trades, window_size=200):
        self.net_worths[current_step] = net_worth
        self.balances[current_step] = balance
        ...
        self.fig.suptitle(f'{self.title} | Balance: {balance} € | Open P/L: {round(net_worth-balance,2)} € | Profit: {profit_percent}%')
        ...
        dates = np.array([date2num(x) for x in date_labels])

        date_labels = pd.to_datetime(self.df['Date'], unit='s').dt.strftime("%d.%m.%Y %H:%M").values[step_range]
